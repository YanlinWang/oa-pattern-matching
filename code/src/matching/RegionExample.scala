...
object RegionExample {
  ...
  trait EvalRegionOriAlg[In <: Eval] extends RegionAlg[In, Eval] {
  ...
  object evalRegionOriAlg extends EvalRegionOriAlg[Eval]
  ...
  trait EvalInv3 extends InvRegion[EvalInv3] with Transform[EvalInv3] with Eval

  trait EvalRegionAlg2[E, In <: InvRegion[In] with Transform[E]] extends PatternRegionAlg[In, Transform[E]] {
  ...
  trait EvalRegionAlg3[E, In <: InvRegion[In] with Transform[In]] extends PatternRegionAlg[In, Transform[In]] {
  ...
  def test1() = {
    ...
    val o = makeRegion(evalRegionOriAlg)
  ...
  def test2() = {
    println(">>> Testing EvalRegionAlg")
      ...
      def pre[In <: InvRegion[In] with Eval] : RegionAlg[In, InvRegion[In] with Eval] =
        regionMerge[In, Eval, InvRegion[In]](mixEvalInv, evalRegionAlg, invRegionAlg)
    ...
    println("Is (0.5,0.5) inside it? " + o.eval(0.5, 0.5))
      ...
      def evalInvAlg = combine[Eval, InvRegion[EvalInv], EvalInv](evalRegionAlg, invRegionAlg)
  ...
  def test4() = { // use combine
      def evalAlg2 : PatternRegionAlg[EvalInv2, Transform[Eval]] = new EvalRegionAlg2[Eval, EvalInv2] {
        val alg : PatternRegionAlg[EvalInv2, Eval] = evalRegionAlg[EvalInv2]
      }
      def evalInvAlg : RegionAlg[EvalInv2, Transform[Eval] with InvRegion[EvalInv2]] =
        combine[Transform[Eval], InvRegion[EvalInv2], EvalInv2](evalAlg2, invRegionAlg)
  ...
  def test5() = { // use merge
    ...
    val evalAlg3 : PatternRegionAlg[EvalInv2, Transform[Eval]] =
      new EvalRegionAlg2[Eval, EvalInv2] {
        val alg : PatternRegionAlg[EvalInv2, Eval] = evalRegionAlg[EvalInv2]
      }
    /*
    val evalAlg4 : PatternRegionAlg[EvalInv3, Transform[EvalInv3]] =
      new EvalRegionAlg3[Eval, EvalInv3] {
        val alg : PatternRegionAlg[EvalInv3, EvalInv3] = evalRegionAlg[EvalInv3]
      }
  */
